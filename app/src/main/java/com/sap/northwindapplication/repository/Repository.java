package com.sap.northwindapplication.repository;
...
import com.sap.northwindapplication.archcomp.SingleLiveEvent;

import com.sap.northwindapplication.repository.OperationResult.Operation;
...
public class Repository<T extends EntityValue> {
     ...
     * @param demoService - OData service
     * @param entitySet - the entity set this repository provides access for
     */
    public Repository(@NonNull DemoService demoService, @NonNull EntitySet entitySet, @Nullable Property orderByProperty) {
    ...
    public void read() {
        ...
        demoService.executeQueryAsync(dataQuery,
            result -> {
    ...
    public void read(EntityValue parent, String navigationPropertyName) {
        ...
        demoService.loadPropertyAsync(navigationProperty, parent, dataQuery,
            () -> {
    ...
    public void create(@NonNull T newEntity, @NonNull StreamBase media) {
        if (newEntity.getEntityType().isMedia()) {
            demoService.createMediaAsync(newEntity, media,
                () -> {
    ...
    public void create(@NonNull T newEntity) {
        ...
        demoService.createEntityAsync(newEntity,
            () -> {
                insertToCache(newEntity, entities);
                if (relatedEntities.size() > 0) {
                    insertToCache(newEntity, relatedEntities);
    ...
    public void update(@NonNull T updateEntity) {
        demoService.updateEntityAsync(updateEntity,
            () -> {
    ...
    public void delete(@NonNull List<T> deleteEntities) {
        ...
        demoService.applyChangesAsync(deleteChangeSet,
            () -> {
    ...
    public void downloadMedia(@NonNull T entity, Action1<byte[]> successHandler,
        ...
        demoService.downloadMediaAsync(entity,
